...
export default function Ones() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['/api/content', { type: 'ones', page: currentPage, limit: 24, ...activeFilters }],
    queryFn: () => {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-gray-900 to-black">
        <p className="text-white text-xl">حدث خطأ في تحميل المحتوى</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-black text-white">
      <div className="container mx-auto px-4 py-6">
        <h1 className="text-3xl font-bold mb-6 text-center bg-gradient-to-r from-red-400 to-red-600 bg-clip-text text-transparent">
          المحتوى المميز
        </h1>
          ...
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mt-6">
              {data?.content?.map((item: Content) => (
                <AkStyleContentCard 
                  key={item.id} 
                  content={item}
                  linkPath={`/${item.type}/${item.id}/${encodeURIComponent(item.title)}`}
                />
              ))}
            </div>
            ...
            {data?.content?.length === 0 && (
              <div className="text-center py-12">
                <p className="text-gray-400 text-lg">لا يوجد محتوى متاح</p>
              </div>
            )}
          </>
        )}
