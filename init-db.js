...
async function initializeDatabase() {
  ...
  try {
    ...
    // إنشاء الفئات
    await pool.query(`
      INSERT INTO categories (name, name_arabic, type) VALUES
      ('Arabic', 'عربي', 'movie'),
      ('Foreign', 'أجنبي', 'movie'),
      ('Hindi', 'هندي', 'movie'),
      ('Turkish', 'تركي', 'movie'),
    ...
    // إنشاء الأنواع
    await pool.query(`
      INSERT INTO genres (name, name_arabic) VALUES
      ('Action', 'أكشن'),
      ('Drama', 'دراما'),
      ('Comedy', 'كوميديا'),
      ('Romance', 'رومانسي'),
    ...
    // إنشاء محتوى تجريبي
    await pool.query(`
      INSERT INTO content (title, title_arabic, description, description_arabic, type, year, language, quality, resolution, rating, duration, poster_url, video_url, is_active) VALUES
